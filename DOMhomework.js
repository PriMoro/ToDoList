...
nombre.innerHTML = nombre.innerHTML + " Priscila";
...
ToDo.prototype.completeToDo = function (){
  this.complete = ! this.complete; // esto hace que se taché lo que está hecho, y si se vuelve a tocar, se borra lo tachado y se
  // deja como incompleto, como antes.
}
...
function buildToDo(todo, index) {
  ...
  let checkbox = document.createElement("input") // creo una variable para el checkbox
  checkbox.setAttribute("type", "checkbox") // le doy el atributo para que sea una checkbox
  checkbox.id = index;
...
function buildToDos(toDos) {
  // Tu código acá:
  let array = toDos.map((el, index) => {return buildToDo(el, index)})
  return array;
}
